...
function ThreeDee_enqueue_scripts_frontend()
{
	...
	if ($condition) {
		...
		/*		wp_enqueue_script( 'ThreeDee-threejs-collada-loader-animation',  plugin_dir_url( __FILE__ ).'ext/threejs/js/loaders/collada/Animation.js', array( 'jquery' ), $ThreeDee_current_version );
		wp_enqueue_script( 'ThreeDee-threejs-collada-loader-animationhandler',  plugin_dir_url( __FILE__ ).'ext/threejs/js/loaders/collada/AnimationHandler.js', array( 'jquery' ), $ThreeDee_current_version );
		wp_enqueue_script( 'ThreeDee-threejs-collada-loader-animation-keyframeanimation',  plugin_dir_url( __FILE__ ).'ext/threejs/js/loaders/collada/KeyFrameAnimation.js', array( 'jquery' ), $ThreeDee_current_version );
		wp_enqueue_script( 'ThreeDee-threejs-collada-loader',  plugin_dir_url( __FILE__ ).'ext/threejs/js/loaders/ColladaLoader.js', array( 'jquery' ), $ThreeDee_current_version );*/
...
function ThreeDee_plugin_redirect()
{
	if (get_option('ThreeDee_do_activation_redirect', false)) {
		...
		if (!isset($_GET['activate-multi'])) {
			wp_redirect(admin_url('admin.php?page=ThreeDee'));
			exit;
		}
	}
}
...
function ThreeDee_add_color_picker($hook)
{
	if (is_admin()) {
		...
		wp_enqueue_script('custom-script-handle', plugins_url('js/ThreeDee-backend.js', __FILE__), array('wp-color-picker'), false, true);
	}
}
